...
export const editCourse = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (thumbnail) {
            // delete old image
            await cloudinary.v2.uploader.destroy(thumbnail.pubilc_id);
            const myCloud = await cloudinary.v2.uploader.upload(thumbnail, {
                folder: "courses"
            });
            data.thumbnail = {
...
export const addAnswer = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!question) {
            return next(new ErrorHandler('invalid question', 501));
        }
        ...
        if (req.user?._id === question.user._id) {
            ...
            await NotificationModel.create({
                user: req.user?._id,
                title: "New Question Reply Reveived",
            ...
            try {
                await sendMail({ email: question.user.email, subject: "Question Reply", template: "question-reply.ejs", data });
            } catch (error: any) {
                return next(new ErrorHandler(error.message, 400));
            }
        }
...
export const addReview = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const notification = {
            ...
            message: `${req.user?.name} has just posted a review for your course ${course?.name}`,
        };
...
export const addReplyToReview = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!review.commentReaplies) {
            review.commentReaplies = [];
        }
        review.commentReaplies?.push(replyData);
